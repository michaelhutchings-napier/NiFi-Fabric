...
dump_receiver_diagnostics() {
  set +e
  echo
  echo "==> Site-to-Site delivery diagnostics"
...
bash "${ROOT_DIR}/hack/prove-site-to-site-metrics.sh" \
...
python3 - "${tmpdir}/receiver-processor.json" "${receiver_processor_name}" "${receiver_processor_log_prefix}" <<'PY'
import json
import sys
...
if properties.get("log-prefix") != target_prefix:
...
if [[ -z "${delivery_ok}" ]]; then
  echo "expected receiver input port ${receiver_input_port_name} to show delivered metrics within ${delivery_timeout_seconds}s" >&2
  exit 1
fi

echo "ok"
echo "PASS: typed Site-to-Site metrics delivery proof succeeded (${delivery_ok})"
