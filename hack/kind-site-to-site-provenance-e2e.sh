...
KIND_CLUSTER_NAME="${KIND_CLUSTER_NAME:-nifi-fabric-site-to-site-provenance}"
...
PLATFORM_PROVENANCE_VALUES_FILE="${PLATFORM_PROVENANCE_VALUES_FILE:-examples/platform-managed-site-to-site-provenance-values.yaml}"
PLATFORM_KIND_VALUES_FILE="${PLATFORM_KIND_VALUES_FILE:-examples/platform-managed-site-to-site-provenance-kind-values.yaml}"
SITE_TO_SITE_AUTHORIZED_IDENTITY="${SITE_TO_SITE_AUTHORIZED_IDENTITY:-O=NiFi-Fabric, CN=nifi-site-to-site-provenance-client}"
SENDER_CLIENT_SECRET_NAME="${SENDER_CLIENT_SECRET_NAME:-nifi-site-to-site-receiver-provenance-client}"
...
dump_diagnostics() {
  set +e
  echo
  echo "==> site-to-site provenance diagnostics after failure at +$(elapsed)s"
...
helm_values_args=(
  ...
  -f "${ROOT_DIR}/${PLATFORM_PROVENANCE_VALUES_FILE}"
...
if [[ "${SKIP_KIND_BOOTSTRAP}" == "true" ]]; then
  phase "Reusing existing kind cluster for Site-to-Site provenance runtime proof"
  configure_kind_kubeconfig
else
  phase "Creating fresh kind cluster for Site-to-Site provenance runtime proof"
...
phase "Proving typed Site-to-Site provenance bootstrap"
bash "${ROOT_DIR}/hack/prove-site-to-site-provenance-delivery.sh" \
  --sender-namespace "${NAMESPACE}" \
  --sender-release "${HELM_RELEASE}" \
  ...
  --sender-expected-input-port "nifi-provenance" \
  ...
  --sender-expected-start-position "endOfStream" \
  --receiver-namespace "${RECEIVER_NAMESPACE}" \
  --receiver-release "${RECEIVER_RELEASE}" \
  --receiver-auth-secret site-to-site-receiver-auth \
  --receiver-expected-authorized-identity "${SITE_TO_SITE_AUTHORIZED_IDENTITY}" \
  --receiver-input-port "nifi-provenance"

print_success_footer "typed Site-to-Site provenance delivery proof completed" \
  ...
  "bash hack/prove-site-to-site-provenance-delivery.sh --sender-namespace ${NAMESPACE} --sender-release ${HELM_RELEASE} --sender-expected-destination-url https://${RECEIVER_RELEASE}.${RECEIVER_NAMESPACE}.svc.cluster.local:8443/nifi --sender-expected-input-port nifi-provenance --sender-expected-auth-type secretRef --sender-expected-authorized-identity '${SITE_TO_SITE_AUTHORIZED_IDENTITY}' --sender-expected-auth-secret-ref-name ${SENDER_CLIENT_SECRET_NAME} --sender-expected-start-position endOfStream --receiver-namespace ${RECEIVER_NAMESPACE} --receiver-release ${RECEIVER_RELEASE} --receiver-expected-authorized-identity '${SITE_TO_SITE_AUTHORIZED_IDENTITY}' --receiver-input-port nifi-provenance"
