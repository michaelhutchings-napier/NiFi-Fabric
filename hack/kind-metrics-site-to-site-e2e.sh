...
KIND_CLUSTER_NAME="${KIND_CLUSTER_NAME:-nifi-fabric-metrics-site-to-site}"
...
dump_diagnostics() {
  set +e
  echo
  echo "==> site-to-site metrics diagnostics after failure at +$(elapsed)s"
...
if [[ "${SKIP_KIND_BOOTSTRAP}" == "true" ]]; then
  phase "Reusing existing kind cluster for Site-to-Site metrics runtime proof"
  configure_kind_kubeconfig
else
  phase "Creating fresh kind cluster for Site-to-Site metrics runtime proof"
...
phase "Proving typed Site-to-Site metrics bootstrap"
bash "${ROOT_DIR}/hack/prove-site-to-site-metrics-delivery.sh" \
  --sender-namespace "${NAMESPACE}" \
  --sender-release "${HELM_RELEASE}" \
