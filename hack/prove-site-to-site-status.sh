...
task_name="fabric-site-to-site-status-export"
ssl_service_name="fabric-site-to-site-status-ssl"
...
dump_diagnostics() {
  set +e
  echo
  echo "==> Site-to-Site status diagnostics"
  ...
  kubectl -n "${namespace}" get configmap "${release}-site-to-site-status" -o yaml || true
  ...
  if [[ -f "${tmpdir}/site-to-site-config.json" ]]; then
    echo
    echo "site-to-site status config contract:"
    cat "${tmpdir}/site-to-site-config.json"
  fi
}

on_error() {
  echo "FAIL: typed Site-to-Site status runtime proof failed" >&2
  dump_diagnostics
  exit 1
}
trap on_error ERR
...
kubectl -n "${namespace}" get configmap "${release}-site-to-site-status" -o jsonpath='{.data.config\.json}' >"${tmpdir}/site-to-site-config.json"
python3 - "${tmpdir}/site-to-site-config.json" "${expected_destination_url}" "${expected_input_port}" "${expected_transport}" "${expected_platform}" "${expected_auth_type}" "${expected_authorized_identity}" "${expected_auth_secret_ref_name}" <<'PY'
import json
import sys

payload = json.load(open(sys.argv[1]))
destination_url, input_port, transport, platform, auth_type, authorized_identity, auth_secret_ref_name = sys.argv[2:]
...
if payload.get("reporting", {}).get("batchSize") != "1000":
    raise SystemExit(f"expected fixed status batchSize '1000', got {payload.get('reporting', {}).get('batchSize')!r}")
if payload.get("reporting", {}).get("includeNullValues") != "false":
    raise SystemExit(f"expected fixed status includeNullValues 'false', got {payload.get('reporting', {}).get('includeNullValues')!r}")
...
for task in payload.get("reportingTasks", []):
    ...
    if component.get("type") == "org.apache.nifi.reporting.SiteToSiteStatusReportingTask":
        matching_type += 1
    if component.get("name") == target:
        target_id = task["id"]
if matching_type != 1:
    raise SystemExit(f"expected exactly one SiteToSiteStatusReportingTask, found {matching_type}")
if not target_id:
...
python3 - "${tmpdir}/reporting-task.json" "${task_name}" "${expected_destination_url}" "${expected_input_port}" "${expected_transport}" "${expected_platform}" <<'PY'
import json
import sys

payload = json.load(open(sys.argv[1]))
task_name, destination_url, input_port, transport, platform = sys.argv[2:]
...
include_null_values = properties.get("include-null-values") or properties.get("Include Null Values")
if str(include_null_values).lower() != "false":
    raise SystemExit(f"expected include-null-values false, got {include_null_values!r}")
if properties.get("Component Type Filter Regex") != "(Processor|ProcessGroup|RemoteProcessGroup|RootProcessGroup|Connection|InputPort|OutputPort)":
    raise SystemExit("unexpected Component Type Filter Regex")
if properties.get("Component Name Filter Regex") != ".*":
    raise SystemExit("unexpected Component Name Filter Regex")
...
if expected_auth_type == "secretRef":
    for key in ("Keystore Filename", "Truststore Filename"):
        value = properties.get(key, "")
        if not value.startswith("/opt/nifi/fabric/site-to-site-status-ssl/"):
...
echo "PASS: typed Site-to-Site status runtime proof succeeded"
