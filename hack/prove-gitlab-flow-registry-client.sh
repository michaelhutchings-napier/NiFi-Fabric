...
dump_diagnostics() {
  set +e
  echo
  echo "==> GitLab Flow Registry diagnostics"
...
on_error() {
  echo "FAIL: GitLab Flow Registry runtime proof failed" >&2
  dump_diagnostics
  exit 1
}
trap on_error ERR
...
if [[ "${provider}" != "gitlab" ]]; then
  echo "client ${client_name} is provider=${provider}, expected gitlab" >&2
  exit 1
fi
...
if not ref:
    raise SystemExit("prepared GitLab client is missing Access Token secret reference")
...
if [[ -z "${token_secret}" || -z "${token_key}" ]]; then
  echo "prepared GitLab client is missing Access Token secret reference" >&2
  exit 1
fi

gitlab_token="$(kubectl -n "${namespace}" get secret "${token_secret}" -o "jsonpath={.data.${token_key}}" | base64 -d)"
if [[ -z "${gitlab_token}" ]]; then
  echo "GitLab token Secret ${token_secret}/${token_key} is empty" >&2
  exit 1
fi
...
if [[ -z "${existing_id}" ]]; then
  python3 - "${tmpdir}/client.json" "${tmpdir}/bundle.json" "${gitlab_token}" >"${tmpdir}/create-registry-client.json" <<'PY'
import json
import sys
...
if missing:
    raise SystemExit(f"missing expected buckets: {missing}; got {buckets}")

print("registry_id=" + payload.get("registryId", ""))
