...
KIND_CLUSTER_NAME="${KIND_CLUSTER_NAME:-nifi-fabric-istio-ambient}"
...
PLATFORM_ISTIO_AMBIENT_VALUES_FILE="${PLATFORM_ISTIO_AMBIENT_VALUES_FILE:-examples/platform-managed-istio-ambient-values.yaml}"
SKIP_KIND_BOOTSTRAP="${SKIP_KIND_BOOTSTRAP:-false}"
FAST_PROFILE="${FAST_PROFILE:-true}"
ISTIO_NAMESPACE="${ISTIO_NAMESPACE:-istio-system}"
ISTIO_TEMP_DIR="${ISTIO_TEMP_DIR:-${TMPDIR:-/tmp}/nifi-fabric-istio-ambient}"
START_EPOCH="$(date +%s)"
...
install_istio_ambient_control_plane() {
  phase "Installing Istio Ambient control plane"
  kubectl create namespace "${ISTIO_NAMESPACE}" >/dev/null 2>&1 || true
  istioctl install -y --set profile=ambient >/dev/null
  kubectl -n "${ISTIO_NAMESPACE}" rollout status deployment/istiod --timeout=5m >/dev/null
  kubectl -n "${ISTIO_NAMESPACE}" rollout status daemonset/ztunnel --timeout=5m >/dev/null
}

prepare_ambient_namespace() {
  phase "Preparing the NiFi namespace for pod-label-based Ambient enrollment only"
  kubectl create namespace "${NAMESPACE}" >/dev/null 2>&1 || true
  kubectl label namespace "${NAMESPACE}" istio-injection- >/dev/null 2>&1 || true
  kubectl label namespace "${NAMESPACE}" istio.io/dataplane-mode- >/dev/null 2>&1 || true
}

verify_ambient_label_without_sidecars() {
  phase "Verifying Ambient pod-label enrollment without sidecars"
  local dataplane_mode containers
  dataplane_mode="$(kubectl -n "${NAMESPACE}" get pod "${HELM_RELEASE}-0" -o jsonpath='{.metadata.labels.istio\.io/dataplane-mode}')"
  containers="$(kubectl -n "${NAMESPACE}" get pod "${HELM_RELEASE}-0" -o jsonpath='{.spec.containers[*].name}')"
  if [[ "${dataplane_mode}" != "ambient" ]]; then
    echo "expected pod ${HELM_RELEASE}-0 to carry istio.io/dataplane-mode=ambient, got: ${dataplane_mode:-<empty>}" >&2
    return 1
  fi
  if [[ "${containers}" == *"istio-proxy"* ]]; then
    echo "expected pod ${HELM_RELEASE}-0 to remain sidecar-free under Ambient, got containers: ${containers}" >&2
    return 1
  fi
}

verify_controller_not_meshed() {
  phase "Verifying the controller remains outside Ambient"
  local containers
...
verify_headless_cross_pod_https() {
  ...
  if [[ -z "${expected_replicas}" || "${expected_replicas}" -lt 2 ]]; then
    echo "expected at least two replicas for the bounded Istio Ambient proof, got ${expected_replicas:-<empty>}" >&2
    return 1
  fi
...
dump_diagnostics() {
  set +e
  echo
  echo "==> Istio Ambient compatibility diagnostics after failure at +$(elapsed)s"
  kubectl config use-context "kind-${KIND_CLUSTER_NAME}" >/dev/null 2>&1 || true
  kubectl config current-context || true
  istioctl version || true
  kubectl -n "${ISTIO_NAMESPACE}" get deploy,daemonset,pod,svc || true
...
helm_values_args=(
  ...
  -f "${ROOT_DIR}/${PLATFORM_ISTIO_AMBIENT_VALUES_FILE}"
)
...
if [[ "${SKIP_KIND_BOOTSTRAP}" == "true" ]]; then
  phase "Reusing existing kind cluster for Istio Ambient compatibility proof"
  configure_kind_kubeconfig
else
  phase "Creating fresh kind cluster for Istio Ambient compatibility proof"
...
phase "Installing product chart managed Istio Ambient profile${profile_label}"
...
print_success_footer "Istio Ambient compatibility proof completed" \
  "make kind-istio-ambient-fast-e2e-reuse" \
  "kubectl -n ${NAMESPACE} get sts ${HELM_RELEASE} -o yaml" \
  "kubectl -n ${NAMESPACE} get pods -o custom-columns=NAME:.metadata.name,LABEL:.metadata.labels.istio\\.io/dataplane-mode,CONTAINERS:.spec.containers[*].name" \
  "kubectl -n ${ISTIO_NAMESPACE} get daemonset ztunnel"
