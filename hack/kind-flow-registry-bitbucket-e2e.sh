...
KIND_CLUSTER_NAME="${KIND_CLUSTER_NAME:-nifi-fabric-flow-registry-bitbucket}"
...
FLOW_REGISTRY_SECRET_NAME="${FLOW_REGISTRY_SECRET_NAME:-bitbucket-flow-registry}"
FLOW_REGISTRY_CLIENT_NAME="${FLOW_REGISTRY_CLIENT_NAME:-bitbucket-flows-kind}"
...
dump_diagnostics() {
  set +e
  echo
  echo "==> Bitbucket Flow Registry evaluator diagnostics"
  ...
  kubectl -n "${NAMESPACE}" get svc bitbucket-mock -o wide || true
  ...
  kubectl -n "${NAMESPACE}" logs deployment/bitbucket-mock --tail=300 || true
}
...
helm_values_args=(
  ...
  -f "${ROOT_DIR}/examples/bitbucket-flow-registry-kind-values.yaml"
)
...
if [[ "${SKIP_KIND_BOOTSTRAP}" == "true" ]]; then
  phase "Reusing existing kind cluster for NiFi ${NIFI_IMAGE##*:} Bitbucket Flow Registry proof"
  configure_kind_cluster_access "${KIND_CLUSTER_NAME}"
  require_reuse_prereqs
else
  phase "Creating fresh kind cluster for NiFi ${NIFI_IMAGE##*:} Bitbucket Flow Registry proof"
  ensure_fresh_cluster
  configure_kind_cluster_access "${KIND_CLUSTER_NAME}"
fi
...
phase "Creating Bitbucket Flow Registry token Secret"
kubectl -n "${NAMESPACE}" create secret generic "${FLOW_REGISTRY_SECRET_NAME}" \
  --from-literal=token=dummytoken \
  --dry-run=client -o yaml | kubectl apply -f -

phase "Deploying Bitbucket-compatible evaluator service"
NAMESPACE="${NAMESPACE}" bash "${ROOT_DIR}/hack/deploy-bitbucket-flow-registry-mock.sh"
...
phase "Installing managed NiFi ${NIFI_IMAGE##*:} release with Bitbucket Flow Registry overlay${profile_label}"
...
phase "Creating and exercising the Bitbucket Flow Registry client"
bash "${ROOT_DIR}/hack/prove-bitbucket-flow-registry-client.sh" \
  --namespace "${NAMESPACE}" \
...
print_success_footer "NiFi ${NIFI_IMAGE##*:} Bitbucket Flow Registry client runtime proof completed" \
  ...
  "kubectl -n ${NAMESPACE} logs deployment/bitbucket-mock --tail=100" \
