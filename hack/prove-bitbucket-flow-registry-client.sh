...
dump_diagnostics() {
  set +e
  echo
  echo "==> Bitbucket Flow Registry diagnostics"
...
on_error() {
  echo "FAIL: Bitbucket Flow Registry runtime proof failed" >&2
  dump_diagnostics
  exit 1
}
trap on_error ERR
...
if [[ "${provider}" != "bitbucket" ]]; then
  echo "client ${client_name} is provider=${provider}, expected bitbucket" >&2
  exit 1
fi

secret_ref="$(python3 - "${tmpdir}/client.json" <<'PY'
import json
import sys

client = json.load(open(sys.argv[1]))
ref = client.get("sensitivePropertyRefs", {}).get("Access Token")
if not ref:
    raise SystemExit("prepared Bitbucket client is missing Access Token secret reference")
...
if [[ -z "${token_secret}" || -z "${token_key}" ]]; then
  echo "prepared Bitbucket client is missing Access Token secret reference" >&2
  exit 1
fi

bitbucket_token="$(kubectl -n "${namespace}" get secret "${token_secret}" -o "jsonpath={.data.${token_key}}" | base64 -d)"
if [[ -z "${bitbucket_token}" ]]; then
  echo "Bitbucket token Secret ${token_secret}/${token_key} is empty" >&2
  exit 1
fi
...
nifi_curl_retry GET /flow/controller-service-types "" 20 2 >"${tmpdir}/controller-service-types.json"
python3 - "${tmpdir}/controller-service-types.json" >"${tmpdir}/web-client-service-type.json" <<'PY'
import json
import sys

payload = json.load(open(sys.argv[1]))
for service_type in payload.get("controllerServiceTypes", []):
    if service_type.get("type") == "org.apache.nifi.web.client.provider.service.StandardWebClientServiceProvider":
        json.dump(service_type["bundle"], sys.stdout)
        sys.exit(0)

raise SystemExit("NiFi runtime does not expose org.apache.nifi.web.client.provider.service.StandardWebClientServiceProvider")
PY

nifi_curl_retry GET /flow/controller/controller-services "" 20 2 >"${tmpdir}/controller-services.json"
existing_web_client_id="$(
  python3 - "${tmpdir}/controller-services.json" <<'PY'
import json
import sys
...
for service in payload.get("controllerServices", []):
    ...
    if component.get("name") == "bitbucket-web-client":
        print(service["id"])
        break
PY
)"

if [[ -z "${existing_web_client_id}" ]]; then
  python3 - "${tmpdir}/web-client-service-type.json" >"${tmpdir}/create-web-client-service.json" <<'PY'
import json
import sys
...
PY
  create_service_payload="$(tr -d '\n' < "${tmpdir}/create-web-client-service.json")"
  nifi_curl_retry POST /controller/controller-services "${create_service_payload}" 20 2 >"${tmpdir}/created-web-client-service.json"
  existing_web_client_id="$(python3 - "${tmpdir}/created-web-client-service.json" <<'PY'
import json
import sys
...
nifi_curl_retry GET "/controller-services/${existing_web_client_id}" "" 20 2 >"${tmpdir}/web-client-service.json"
python3 - "${tmpdir}/web-client-service.json" >"${tmpdir}/enable-web-client-service.json" <<'PY'
import json
import sys
...
json.dump(enable, sys.stdout)
PY
enable_payload="$(tr -d '\n' < "${tmpdir}/enable-web-client-service.json")"
nifi_curl_retry PUT "/controller-services/${existing_web_client_id}/run-status" "${enable_payload}" 20 2 >"${tmpdir}/enabled-web-client-service.json"

nifi_curl_retry GET /controller/registry-types "" 20 2 >"${tmpdir}/registry-types.json"
...
nifi_curl_retry GET /flow/registries "" 20 2 >"${tmpdir}/flow-registries.json"
existing_id="$(
...
if [[ -n "${existing_id}" ]]; then
  nifi_curl_retry GET "/controller/registry-clients/${existing_id}" "" 20 2 >"${tmpdir}/existing-registry-client.json"
  existing_version="$(python3 - "${tmpdir}/existing-registry-client.json" <<'PY'
import json
import sys

print(json.load(open(sys.argv[1]))["revision"]["version"])
PY
)"
  nifi_curl_retry DELETE "/controller/registry-clients/${existing_id}?version=${existing_version}&disconnectedNodeAcknowledged=false" "" 20 2 >"${tmpdir}/deleted-registry-client.json"
  existing_id=""
fi

if [[ -z "${existing_id}" ]]; then
  python3 - "${tmpdir}/client.json" "${tmpdir}/bundle.json" "${bitbucket_token}" "${existing_web_client_id}" >"${tmpdir}/create-registry-client.json" <<'PY'
import json
import sys

client_path, bundle_path, token, web_client_id = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
client = json.load(open(client_path))
bundle = json.load(open(bundle_path))
properties = dict(client["properties"])
properties["Access Token"] = token
properties["Web Client Service"] = web_client_id
...
PY
  ...
  nifi_curl_retry POST /controller/registry-clients "${create_payload}" 20 2 >"${tmpdir}/created-registry-client.json"
...
nifi_curl_retry GET "/flow/registries/${registry_id}/buckets" "" 20 2 >"${tmpdir}/buckets-response.txt"
