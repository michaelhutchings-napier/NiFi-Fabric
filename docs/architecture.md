...
- for declared prepared `provider=nifiRegistry` entries only, the bounded import path can also create and reconcile the exact live Flow Registry Client object it needs without broadening into generic registry-client management
...
- runtime-managed import reuses live Flow Registry Client support and selected-version content exposed through NiFi, while still keeping broad live registry-client lifecycle and provider write-back out of scope
